...
async function renderResults() {
	try {
		const data = await currentUser.getNews(page,inputQuery.value);
		if (data.totalResults === undefined) throw 'Error loading data!';
		...
		nav.style.display = data.totalResults ? 'block': 'none';
...
////////////////////Render kết quả khi click button 'Search'

btnSubmit.addEventListener('click', function() {
	if (inputQuery.value.trim() === '') {//Nếu người dùng chưa nhập nội dung tìm kiếm hoặc nội dung toàn khoản trắng thì báo lỗi
